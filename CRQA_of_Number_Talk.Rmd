...
author: "Shirley Duong (shd77@pitt.edu)"
...
This analysis is presented in the article, "It takes two to tango: Exploring the dynamic structures of caregiver-child conversations using recurrence quantification."  


**General steps**:

* Describe the sample of parent-child dyads.
* Set up (e.g., determine hyperparameters) and run CRQA on parent-child number talk.
* Generate recurrence plots of several examples.
* Impute child assessment data.
* Estimate regression models relating the CRQA-derived measures and child outcomes.   

First, import the necessary libraries to carry out the steps above.

```{r import libraries, echo=TRUE, results='hide'}
# data manipulation
library(tidyverse)  
library(reshape) # used to melt data to generate recurrence plot
...
Parents in this sample were mostly White and non-Hispanic, followed by  Black, Asian, and multiracial or another race or ethnicity.   
...
```{r describe sample education income}
o %>% dplyr::select(parent_education_years, family_yearly_income) %>%
...
Event series variables: 

* `preID`: Dyad identifier
* `timepoint`: The order in which the parent or child utterance appeared in the conversation; takes on values of 1 to the total number of utterances across speakers in each interaction
* `source`: Speaker, **p** = *parent*, **c** = *child*
* `parent_ts`: Event series of parent utterances, coded as **1** = *engaging in number talk*, **2** = *non-number talk*, **3** = *parent non-number talk*, or **5** = *no utterance of the parent during non-NT of the child*
* `child_ts`: Event series of child utterances, coded as **1** = *engaging in number talk*, **2** = *non-number talk*, **4** = *child non-number talk*, or **6** = *no utterance of the child during non-NT of the parent*  
...
Specify CRQA hyperparameters. For this analysis, we used a radius (`radius` argument in `crqa()`) of .5, a delay (`delay`) of 1, and an embedded dimension (`embed`) of 1.
...
Use the `crqa()` function to run the recurrence analysis.


## Example data

Run CRQA on a subset of one parent-child conversation to show the process and output. The output contains 10 recurrence measures values and 1 matrix indicating recurrent points with lines. This example corresponds to Table 2 in the article.  
...
Apply `crqa()` to the case study data for each parent-child dyad using `group_by()`. Extract the **recurrence rate** (`RR`), **percent determinism** (`DET`), **mean diagonal line** (`meanL`), **laminarity** (`LAM`), and **trapping time** (`TT`) by accessing attributes of the CRQA output with `$`. Assign to a new dataframe where each row corresponds to one parent-child dyad. 
...
CRQA-derived measures:

* **Recurrence rate** (`RR`): Percentage of recurrent points in a RP. How often is number talk co-occurring or being reciprocated? 
* **Determinism** (`DET`): Percentage of recurrent points that form diagonal lines. How often do parents and children follow each others’ number utterances or engage in “back and forth” number exchanges?
* **Average diagonal line** (`meanL`) - Mean length of diagonal lines.
* **Laminarity** (`LAM`) – Percentage of recurrence points that form vertical (or horizontal) lines. How often does one speaker use consecutive number utterances after the other person introduces or uses number talk?
* **Trapping time** (`TT`) - Average length of a laminar phase (vertical or horizontal lines).  


Obtain descriptive statistics of the CRQA-derived measures using the `describe()` function from the 'psych' package. This corresponds to Table 3 in the article.  
...
Visualize the distribution of the measures.
...
Generate recurrence plots (RPs) for **six** conversations: one with a relatively high RR, one with RR = 0 (but including parent number talk), one with a low RR, and four with differing DET and/or LAM but relatively similar frequencies of parent number talk utterances.  

Reading the plots:

* The parent event series is plotted along the x-axis and the child event series is plotted along the y-axis. 
* Each blue (parent) or red (child) rug marks (thin lines) along the axes represent whether a number utterance occurred at a particular point in time. This can be summarized to show the *frequency* of number talk.
* The purple areas on the plot represent when number talk co-occurs. This can be summarized to show the *reciprocity* of number talk.
...
First, extract the RP from the example and reshape the data so that there are 3 columns: the parent event series, the child event series, and `value` which indicates whether each pair of parent-child utterances is a recurrent point.  
...
Then, use ggplot's `geom_raster()` to generate the RP.
```{r rp example raster}
# represents the CO-OCCURRENCE of number talk 
...
Generate the data for the rugs (showing the occurrence of number talk) and then add to the plot.  
```{r rp example rugs}
...
# representing the OCCURRENCE of number talk 
...
Write a function to generate RPs without repeating the code 6 times.  
...
make_recurrence_plot <- function(data, chosen_dyad){
  ...
  if(is.logical(class(crqa_output$RP))){ # if RR = 0...
    
    # the crqa() function does not supply an RP if there is no recurrence
    # note: in these cases, the class of $RP is logical, not a matrix
    # so we need to supply an RP for these cases
    # it needs to have the same dimensions as the total number of utterances in the corresponding conversation
...
Total parent utterances = 134, total child utterances = 56, parent number talk (NT) = 50, child NT = 27, RR = 7.33, DET = 33.18, meanL = 2.39, LAM = 34.47, TT = 2.44. Figure 4 in the article.  
...
Only one person used number talk. This was the case for **5** dyads in the sample. Total parent utterances = 112, total child utterances = 112, parent NT = 6, child NT = 0. Figure 5 in the article.  
...
```{r vis missing data}
visdat::vis_miss(all_data %>%
                   dplyr::select(RR, 
                                 child_math, child_vocabulary, child_executive_function))
```


Perform imputation using `mice()` from the 'mice' package. First, set the prediction matrix, which indicates which variables are going to be used to predict the values of the missing data.  

```{r setup imputation}
# make copy of dataset
all_data_use <- all_data

# create several complete datasets drawing from the distribution of the existing data
...
predM[, c("total_parent_utterances")]=0
predM[, c("total_child_utterances")]=0
predM[, c("parent_number_utterances")]=0
predM[, c("child_number_utterances")]=0
predM[, c("all_number_utterances")]=0
```


Impute missing data and store imputed datasets in one list for later use in pooled regressions.  
```{r run imputation}
...
Use handy correlation matrix function from [http://www.sthda.com/](http://www.sthda.com/). I know that this website is the source of this function but I cannot find the link to the exact page that I got this from - Sorry!
...
Three CRQA measures (the percentage metrics), RR, DET, and LAM, are added in step 2. Shown in table 6 in the article.   

**Overall, RR is significantly related to child math, but not child vocabulary or executive function. All other recurrence measures are not significantly associated to any child outcomes.**  
